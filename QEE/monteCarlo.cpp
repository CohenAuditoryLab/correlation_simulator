...
double getMaxError(const Vector &q, const Vector &p, const IntVector &nz, double maxPrecision) {
    ...
    for (int n=0;n<nz.size();n++) {
        ...
        for (int k=0;k<q[idx].size();k++) {
            
            double gradError = pow(q[idx][k] - p[idx][k], 2.0);
            
            if (q[idx][k]<maxPrecision) gradError /= (1 - maxPrecision);
            else                        gradError /= (maxPrecision * q[idx][k] * (1 - q[idx][k]));
    ...
    return ( maxError / sqrt(2 * log(N*(N+1)/2) ) );
...
void getFixedSample(const Vector &expJ, const IntVector &neighbor, const std::vector<int> &fixedSites, MT::MersenneTwist &mt, int N, int numSteps, int stepSize, std::vector<int> &lattice, std::vector<int> &nonzero, Vector &p, Vector &expH, std::vector<double> &sumExpH) {
    ...
    for (int n=0;n<numSteps;n++) {
        ...
        for (int k=0;k<stepSize;k++) {
            ...
            bool allowed = true;
            for (int i=0;i<fixedSites.size();i++) { if (fixedSites[i]==k%N) allowed = false; }
            
            if (allowed) dynamicsRF(expJ, neighbor, k%N, mt.genrand_real1(), lattice, nonzero, expH, sumExpH);
        ...
        int nmut   = 0;
        for (int i=0;i<N;i++) { if (nonzero[i]) nmut++; }
        ...
        if (nmut>mutCut) {
            ...
            for (int i=0;i<lattice.size();i++) {
                ...
                bool allowed = true;
                for (int k=0;k<fixedSites.size();k++) { if (fixedSites[k]==i) allowed = false; }
                
                if (allowed) { lattice[i] = (int) p[i].size(); nonzero[i] = 0; }
...
// Write Monte Carlo sample to file with some sites fixed

void writeFixedSample(FILE* output, const Vector &expJ, const IntVector &neighbor, const std::vector<int> &fixedSites, MT::MersenneTwist &mt, int N, int numSteps, int stepSize, std::vector<int> &lattice, std::vector<int> &nonzero, Vector &p, Vector &expH, std::vector<double> &sumExpH) {
    ...
    for (int n=0;n<numSteps;n++) {
        ...
        for (int i=0;i<lattice.size();i++) fprintf(output,"%d ",lattice[i]);
        //for (int i=0;i<lattice.size();i++) { if (nonzero[i]) fprintf(output,"%d %d\t",i,lattice[i]); }
        fprintf(output,"\n");
...
void getError(const Vector &q, const Vector &J, int N, double B, int numSteps, int numRuns, double gamma, double alpha, std::vector<double> &error) {
    ...
    error[1] = epsilonC(q, p, N, maxPrecision, J, gamma);
    error[2] = getMaxError(q, p, maxPrecision, J, gamma, alpha);
...
void getErrorMCLearn(const Vector &q, const Vector &J, Vector &expJ, double B, int numSteps, int numRuns, double gamma, double alpha, Vector &p, std::vector<double> &error, std::vector<int> &latticeStart) {
    ...
    error[0] = epsilonP( q, p, N, maxPrecision, J, gamma, alpha);
    error[1] = epsilonP2(q, p, N, maxPrecision, J, gamma);
    error[2] = getMaxError( q, p, maxPrecision, J, gamma, alpha);
...
void getEpitopeCorrelations(const Vector &J, const Vector &expJ, const std::vector<std::vector<int> > &epitopeStates, const std::vector<int> &epitopeStartSite, int numSteps, int numRuns, Vector &p, std::vector<int> &latticeStart) {
    ...
    for (int i=0;i<epitopeStartSite.size();i++) { for (int j=0;j<epitopeStates[i].size();j++) {
    
        fixedSites.push_back(epitopeStartSite[i]+j);
    ...
    for (int m=0;m<numRuns;m++) {
        ...
        for (int i=0;i<lattice.size();i++) {
            ...
            if (lattice[i]==J[i].size()) nonzero[i]=0;
            else                         nonzero[i]=1;
        ...
        // Fix epitope region
        int k=0;
        for (int i=0;i<epitopeStates.size();i++) { for (int j=0;j<epitopeStates[i].size();j++) {
        
            lattice[fixedSites[k]]=epitopeStates[i][j];
            
            if (lattice[fixedSites[k]]==J[fixedSites[k]].size()) nonzero[fixedSites[k]]=0;
            else                                                 nonzero[fixedSites[k]]=1;
...
// Get MC sample of Potts configurations using rejection-free Monte Carlo with epitope sites fixed

void getEpitopeStates(FILE *output, const Vector &J, const Vector &expJ, const std::vector<std::vector<int> > &epitopeStates, const std::vector<int> &epitopeStartSite, int numSteps, int numRuns, Vector &p, std::vector<int> &latticeStart) {
    ...
    for (int m=0;m<numRuns;m++) {
        ...
        setEnergies(expJ, neighbor, lattice, nonzero, expH, sumExpH);
        writeFixedSample(output, expJ, neighbor, fixedSites, mt, N, numSteps, stepSize, lattice, nonzero, p, expH, sumExpH);
